...
public class ExchangeRateConfiguration : IEntityTypeConfiguration<ExchangeRate>
{
    public void Configure(EntityTypeBuilder<ExchangeRate> builder)
    {
        ...
        builder.HasIndex(x => new { x.BaseCurrency, x.TargetCurrency, x.ClientGroupId, x.IsActive })
            .HasDatabaseName("IX_ExchangeRates_GroupRates")
            .HasFilter("[ClientGroupId] IS NOT NULL AND [IsActive] = 1");
        ...
        builder.HasCheckConstraint("CK_ExchangeRates_EffectiveDates", "[EffectiveTo] IS NULL OR [EffectiveTo] > [EffectiveFrom]");

        builder.HasCheckConstraint("CK_ExchangeRates_RateTypeConstraints",
            "([Type] = 'General' AND [ClientId] IS NULL AND [ClientGroupId] IS NULL) OR " +
            "([Type] = 'Group' AND [ClientId] IS NULL AND [ClientGroupId] IS NOT NULL) OR " +
            "([Type] = 'Individual' AND [ClientId] IS NOT NULL AND [ClientGroupId] IS NULL)");
    }
}
...
//        builder.HasOne<ClientGroup>()
//            .WithMany(g => g.ExchangeRates)
//            .HasForeignKey(x => x.ClientGroupId)
//            .IsRequired(false)
//            .OnDelete(DeleteBehavior.Restrict);

//        // Remove old ClientGroup string property
//        builder.Ignore(x => x.ClientGroup); // If you had this as a string

//        builder.Property(x => x.ClientGroupId)
//            .IsRequired(false);

//        // Update indexes to use ClientGroupId instead of ClientGroup string
//        builder.HasIndex(x => new { x.BaseCurrency, x.TargetCurrency, x.ClientGroupId, x.IsActive })
//            .HasDatabaseName("IX_ExchangeRates_GroupRates")
//            .HasFilter("[ClientGroupId] IS NOT NULL AND [IsActive] = 1");
